...
import org.dynamic.config.key.Key;
...
public class HttpConnectionPool {
    ...
    public synchronized void initConnectionPool() {
    	...
    	try {
			requestQueueSize = Integer.parseInt(configService.getConfigValue("FaultCanister", connectionPoolKey.getKeyString(configService.REQUEST_QUEUE_SIZE), ConfigService.TYPE_PROPERTY));
		} catch (ConfigServiceException e) {
			// if exception, use default values, just logging error here
			//Logger.warn("Could not retrieve config values", e);
		}
    	try {
			maxConnections = Integer.parseInt(configService.getConfigValue("FaultCanister", connectionPoolKey.getKeyString(configService.MAX_CONNECTIONS), ConfigService.TYPE_PROPERTY));
		} catch (ConfigServiceException e) {
			// if exception, use default values, just logging error here
			//Logger.warn("Could not retrieve config values", e);
		}
	...
	/**
	 * @return the connectionPoolKey
	 */
	public Key getConnectionPoolKey() {
		return connectionPoolKey;
	}

	/**
	 * @param connectionPoolKey the connectionPoolKey to set
	 */
